...
public class RocchioRelevance {
	...
	public static void main(String[] args) throws IOException {
		...
		System.out.println(" ");
		System.out.print("Enter Rank of Relevant Documents with space : ");
		Scanner feedBack = new Scanner(System.in);
		String feedbackInput = feedBack.nextLine();
		String[] feed = feedbackInput.split("\\s+");
		int[] array = new int[feed.length];
		int[] RelDocArray = new int[array.length];
		
		System.out.println("Calculating...........");
			for(int i=0; i<feed.length; i++){
				array[i] = Integer.parseInt(feed[i]);
				System.out.println(" ");
	...
	//Calculating NonRelevantDocuments Vector Sum.
	  ...
	  int newQuerySort[] = new int[TermFreqHash.size()];
		...
		System.out.println(" ");
		System.out.print("The above Ranking is for New Query : " + " ");
		for(int i=0; i<20; i++){
